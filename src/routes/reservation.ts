...
router.post("/expire/all", async (req, res) => {
    console.log("Expire all reservations requested")
    try {
        const expired = await expireAllReservations()
        res.json({ 
            message: `${expired.length} reservation(s) expired`,
...
router.post("/expire/selected", async (req, res) => {
    const { labels } = req.body
    ...
    try {
        const expired = await expireSelectedReservations(labels)
        res.json({ 
            message: `${expired.length} reservation(s) expired`,
            count: expired.length,
            labels: labels
